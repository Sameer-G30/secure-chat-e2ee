...
export interface EnvelopeMetadata {
  ...
  senderId: string
  // Bind a client-chosen message identity into the AD (v2 only). Present on every
  // new send and every edit; omitted entirely reproduces the original v1 format
  // exactly, so existing history keeps decrypting unchanged. Generated by the
  // sender *before* encryption (never by the server), because it is authenticated
  // inside the ciphertext tag.
  messageId?: string
  // Count edits to one v2 message: 0 for the original send, 1 for the first edit,
  // and so on. Bound into the v2 AD alongside messageId so the server can only
  // ever serve the current revision as authentic — a stale ciphertext from an
  // earlier revision fails verification against the current revision's AD. This
  // is what closes the edit-rollback gap a naive edit feature would introduce.
  revision?: number
}
...
function encodeAssociatedData(
  ...
  // A present messageId means this is a v2 send (new message or edit); its
  // absence reproduces the original v1 array exactly, so history encrypted
  // before message editing existed keeps decrypting unchanged forever.
  const canonicalMetadata =
    metadata.messageId === undefined
      ...
      : JSON.stringify([
          // v2 additionally binds message identity and revision, closing the
          // edit-rollback gap: a stale ciphertext from an earlier revision
          // cannot be replayed as though it were the current one.
          'secure-chat-envelope-v2',
          metadata.conversationId,
          metadata.senderId,
          keyEpoch,
          metadata.messageId,
