...
- **A (not competitive):** NO
  Chat-eval extra FN vs TF-IDF = 4 (need ≥ 10); A_chat=NO.
  TEST scam-recall gap vs DistilBERT = -0.0060 (need ≥ 0.05); A_test=NO.
- **B (link-heavy extra misses):** NO
  URL-related fraction on chat_eval = 0.000 (need > 0.50). Extra chat-eval FN = 4; URL-related = 0.
- **C (URL concat did not close the gap):** NO
  Chat URL-recall gap vs TF-IDF ref = 0.0000; TEST URL-recall gap = -0.0065 (material if ≥ 0.05).
