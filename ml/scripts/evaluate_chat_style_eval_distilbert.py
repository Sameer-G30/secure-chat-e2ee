"""Score a trained DistilBERT checkpoint on the locked chat-style eval set.

Loads the frozen threshold from reports/distilbert/test_metrics.json (chosen
on VALIDATION only) and the checkpoint under models/distilbert/. Never fits
or retunes on data/chat_eval/chat_style_eval_v1.csv.

Usage (from ml/):
    uv run python scripts/evaluate_chat_style_eval_distilbert.py
"""
...
# Import os so tokenizers stay single-threaded under WSL2.
import os
...
# Default to LLM chat-register CSVs, matching train_distilbert.py.
_DEFAULT_PROCESSED_NAME = "processed_chat_llm"

# The locked evaluation-only file; never passed to fine_tune().
_DEFAULT_CHAT_EVAL_PATH = Path("data/chat_eval/chat_style_eval_v1.csv")

# Read frozen threshold from the TEST report written by train_distilbert.py.
_DEFAULT_REPORTS_DIR = Path("reports/distilbert")

# Default checkpoint location written by train_distilbert.py.
...
def parse_args() -> argparse.Namespace:
    ...
    # Reports directory containing the frozen DistilBERT test_metrics.json.
    parser.add_argument(
        "--reports-dir",
        type=Path,
        default=_DEFAULT_REPORTS_DIR,
        help="Directory with test_metrics.json (default: reports/distilbert).",
    )
    # Checkpoint directory written by train_distilbert.py.
    parser.add_argument(
        "--model-dir",
        type=Path,
        default=_DEFAULT_MODEL_DIR,
        help="Fine-tuned checkpoint directory (default: models/distilbert).",
    )
...
def load_frozen_choices(reports_dir: Path) -> dict:
    """Return the DistilBERT test_metrics.json payload, failing if untrained."""
    ...
    if not metrics_path.exists():
        ...
        raise FileNotFoundError(
            f"No frozen DistilBERT report at {metrics_path}. "
            "Run scripts/train_distilbert.py first; this script must not retune "
    ...
    if "chosen_threshold" not in payload:
        # Ask for a regenerated report from the Slice 5 training protocol.
        raise ValueError(
            f"{metrics_path} is missing chosen_threshold. "
            "Re-run scripts/train_distilbert.py with the 70/20/10 protocol."
        )
...
def main() -> None:
    """Score the locked chat-style eval set with a frozen DistilBERT checkpoint."""
    ...
    # Read max_length so tokenization matches the reported model.
    max_length = int(frozen.get("max_length", DEFAULT_MAX_LENGTH))
    ...
    # Reconstruct device/fp16 the same way training did.
    device, used_fp16, _reason = resolve_training_device()

    # Load tokenizer + classifier from the local checkpoint (no Hub required).
    model, tokenizer = load_saved_classifier(args.model_dir)
    # Load the locked eval set; predict_scam_proba never calls fine_tune.
        ...
        json.dumps(
            {
                ...
                "threshold_source": "reports/distilbert/test_metrics.json (validation-frozen)",
