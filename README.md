...
- The server can observe metadata such as account identities, conversation membership, message timing, message size, non-secret epoch numbers, typing indicators, and online/offline presence. Draft text is never sent with typing frames. Epoch rotation broadcasts only the new integer.
...
- Static long-term X25519 keys plus epoch derivation provide key separation, not true forward secrecy. Scheduled rotation changes the subkey id going forward; compromising the static master session key still derives every past and future epoch. A complete Double Ratchet is intentionally out of scope.
...
Tokens stay in memory only: refreshing a tab requires logging in again. Multi-device key recovery is still unsupported (`IdentitySetupError`).

**How to demo an epoch bump** (production default is 50 messages, which is slow in two tabs):

1. Set `EPOCH_ROTATE_AFTER_MESSAGES=2` in `.env` (and restart the API). Leave `EPOCH_ROTATE_AFTER_HOURS=24`. There is no unauthenticated rotate endpoint.
2. Send two messages in the conversation. After the second persist, both tabs should show status `epoch 1`, and the next send's WebSocket frame should have `key_epoch: 1`.
...
- **Backend:** scheduled epoch rotation on the existing WebSocket persist path (CORS stays `GET`/`POST`). Policy: increment `conversations.current_epoch` after **50** persisted messages since the last bump **or 24h** since `last_rotated_at` (NULL treated as `created_at`). Atomic SQL increment plus `last_rotated_at` (Alembic `a9f3c6e12b80`). After a qualifying persist, broadcast `{type: "epoch", current_epoch: <int>}` to every connected member, including the sender. Still reject `key_epoch > current_epoch`; still accept `key_epoch <= current_epoch`. No decrypt, no epoch-key storage, no delete-old-keys. Tests force N=2 (and isolate the 24h half) via env; production default stays 50. API version **0.8.0**.
...
**Conversation-window scoring (ChatScreen policy, not a retrain).** The published ONNX graphs still score one text string. ChatScreen now concatenates the last **6** verified turns (`Me:` / `Them:`, current DM last, oldest dropped first if over ~900 characters) so a split scam ("hey" → "it's me" → "send this") is visible to the same TF-IDF / DistilBERT / LSTM checkpoint. Only trivial short lines **with no URL** (≤ 8 characters, no digits; `ok`, `lol`, `thanks`) skip inference; a shortener still scores. The banner uses `max(frozen sidecar threshold, 0.35)` so TF-IDF Best's 0.20 VAL cut does not flood ordinary DMs. Sidecar JSON and `/?mlLoadCheck=1` fixture banners are unchanged. Banner-cache schema is v2 so old isolated-DM flags are not reused.
